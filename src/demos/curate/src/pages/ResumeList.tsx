...
import { Plus, Search, Filter, MoreVertical, Copy, Trash2, Clock, Sparkles } from 'lucide-react';
...
export function ResumeList() {
  ...
  const text = lang === 'pt'
    ? {
        ...
        subtitle: 'Gerencie e organize todas as versoes do seu curriculo.',
        ai: 'Criar com IA',
        create: 'Criar Novo',
        search: 'Buscar curriculos...',
        filter: 'Filtrar',
        sort: 'Ordenar: Recentes',
        edit: 'Editar Curriculo',
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm">{text.sort}</Button>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <div className="aspect-[3/4] overflow-hidden bg-zinc-100 relative">
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center space-y-2 p-4">
                <Link to={curate(`builder/${resume.id}`)} className="w-full">
                  <Button size="sm" className="w-full bg-white text-zinc-900 hover:bg-zinc-100">{text.edit}</Button>
                </Link>
                <Link to={curate(`preview/${resume.id}`)} className="w-full">
                  <Button size="sm" variant="secondary" className="w-full">{text.preview}</Button>
                </Link>
              </div>
            </div>
        ...
        <Link to={curate('templates')} className="flex aspect-[3/4] flex-col items-center justify-center rounded-xl border-2 border-dashed border-zinc-200 bg-white hover:border-zinc-300 hover:bg-zinc-50 transition-all">
          ...
          <p className="font-bold text-zinc-900 text-sm">{text.create}</p>
        </Link>
      </div>
    </div>
  );
