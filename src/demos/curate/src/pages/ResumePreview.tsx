    ...
    <div className="min-h-screen bg-zinc-100 flex flex-col">
      ...
      <header className="sticky top-6 z-50 flex h-16 items-center justify-between border-b border-zinc-200 bg-white px-4 sm:px-6 lg:px-8 shadow-sm">
        <div className="flex items-center space-x-4">
          ...
          <h1 className="text-sm font-bold text-zinc-900">{resume.title} — Preview</h1>
        </div>
        ...
        <div className="w-full max-w-[850px] bg-white shadow-2xl p-12 sm:p-20 font-serif min-h-[1100px]">
          <div className="space-y-10">
            ...
            <section className="space-y-4">
              <h2 className="text-lg font-bold uppercase tracking-widest text-zinc-900 border-b-2 border-zinc-100 pb-2">Professional Summary</h2>
              <p className="text-base leading-relaxed text-zinc-700">{resume.personalInfo.summary}</p>
            </section>

            {/* Experience */}
            <section className="space-y-6">
              <h2 className="text-lg font-bold uppercase tracking-widest text-zinc-900 border-b-2 border-zinc-100 pb-2">Work Experience</h2>
              {resume.experience.map(exp => (
                <div key={exp.id} className="space-y-2">
                  <div className="flex justify-between items-baseline">
                    <h3 className="text-xl font-bold text-zinc-900">{exp.position}</h3>
                    <span className="text-sm font-medium text-zinc-500">{exp.startDate} — {exp.endDate || 'Present'}</span>
                  </div>
            ...
            {/* Education */}
            <section className="space-y-6">
              <h2 className="text-lg font-bold uppercase tracking-widest text-zinc-900 border-b-2 border-zinc-100 pb-2">Education</h2>
              {resume.education.map(edu => (
                <div key={edu.id} className="space-y-1">
                  ...
                  <p className="text-lg text-zinc-600 italic">{edu.degree} in {edu.field}</p>
                </div>
              ))}
            </section>
            ...
            {/* Skills */}
            <section className="space-y-4">
              <h2 className="text-lg font-bold uppercase tracking-widest text-zinc-900 border-b-2 border-zinc-100 pb-2">Expertise</h2>
