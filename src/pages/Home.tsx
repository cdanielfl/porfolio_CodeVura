  ...
  return (
    <main className="pb-20 pt-10 sm:pb-24 sm:pt-14">
      <section className="relative overflow-hidden">
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:gap-10 sm:px-6 lg:grid-cols-[1.18fr_0.82fr] lg:items-center lg:px-8">
          <div className="relative z-10 pt-11 sm:pt-14 lg:pt-24">
            <motion.span
            ...
            <motion.h1
              ...
              className="mt-5 text-3xl font-semibold leading-[1.08] text-white sm:mt-6 sm:text-5xl lg:text-[3.35rem]"
            >
            ...
            <motion.p
              ...
              className="mt-5 max-w-2xl text-base leading-relaxed text-slate-300 sm:mt-7 sm:text-lg"
            >
            ...
            >
              ...
              <a
                href="#processo"
                className="inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/5 px-5 py-3 text-sm font-semibold text-slate-100 transition-colors hover:bg-white/10 sm:px-6 sm:py-3.5"
              >
            ...
            <motion.div
              ...
              className="mt-7 grid grid-cols-3 gap-2.5 sm:mt-10 sm:max-w-xl sm:gap-3"
            >
              {heroKpis.map((kpi) => (
                <div key={kpi.label} className="rounded-xl border border-white/10 bg-slate-900/50 p-3 backdrop-blur-xl sm:p-4">
          ...
          >
            ...
            <div className="relative overflow-hidden rounded-3xl border border-white/15 bg-slate-900/65 p-3 shadow-[0_28px_90px_-35px_rgba(15,23,42,1)] backdrop-blur-2xl sm:p-6">
              <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(99,102,241,0.16),transparent_35%,rgba(56,189,248,0.08))]" />
              <div className="relative">
                <div className="flex items-center justify-between rounded-2xl border border-white/10 bg-slate-950/70 px-3 py-2.5 text-xs text-slate-300 sm:px-4 sm:py-3">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-3 sm:p-4">
                    <p className="text-xs uppercase tracking-[0.14em] text-slate-400">{t('home.hero.mockup.panelTitle')}</p>
                    <p className="mt-2 text-xl font-semibold text-white sm:text-2xl">{t('home.hero.mockup.panelValue')}</p>
                    <div className="mt-3 h-16 rounded-xl bg-[linear-gradient(180deg,rgba(99,102,241,0.35),rgba(15,23,42,0.05))] sm:mt-4 sm:h-20" />
                  </div>
                  <div className="rounded-2xl border border-white/10 bg-slate-900/80 p-3 sm:p-4">
                    <p className="text-xs uppercase tracking-[0.14em] text-slate-400">{t('home.hero.mockup.roiTitle')}</p>
                    <p className="mt-2 text-xl font-semibold text-white sm:text-2xl">{t('home.hero.mockup.roiValue')}</p>
                    <p className="mt-3 text-sm text-slate-300">{t('home.hero.mockup.roiDescription')}</p>
                  </div>
                </div>

                <div className="mt-4 rounded-2xl border border-white/10 bg-slate-900/80 p-3 sm:p-4">
      ...
      <section id="processo" className="mx-auto mt-16 max-w-7xl px-4 sm:mt-24 sm:px-6 lg:px-8">
        <div className="rounded-3xl border border-white/10 bg-slate-900/55 p-5 backdrop-blur-xl sm:p-10">
          <div className="grid gap-5 md:grid-cols-3">
            {(t('home.results.items', { returnObjects: true }) as HeroKpi[]).map((item) => (
              <div key={item.label} className="rounded-2xl border border-white/10 bg-slate-950/60 p-4 sm:p-5">
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 sm:mt-24 sm:px-6 lg:px-8">
        <div className="mb-8 sm:mb-10">
          ...
          <h2 className="mt-3 text-2xl font-semibold text-white sm:text-4xl">{t('home.services.title')}</h2>
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-300 sm:mt-4 sm:text-base">{t('home.services.subtitle')}</p>
        </div>
        ...
        <div className="grid gap-4 sm:gap-5 md:grid-cols-2 xl:grid-cols-4">
          {services.map((service, index) => (
            <motion.article
              ...
              className="group rounded-2xl border border-white/10 bg-slate-900/60 p-4 transition-all hover:-translate-y-1 hover:border-violet-400/45 hover:bg-slate-900/75 sm:p-6"
            >
              <div className="inline-flex rounded-xl border border-violet-300/30 bg-violet-500/10 p-2.5 text-violet-200">
                {serviceIcons[index]}
              </div>
              <h3 className="mt-4 text-lg font-semibold text-white sm:mt-5 sm:text-xl">{service.title}</h3>
              <p className="mt-3 text-sm leading-relaxed text-slate-300">{service.description}</p>
              <p className="mt-4 border-t border-white/10 pt-4 text-sm font-medium text-sky-300 sm:mt-5 sm:pt-5">{service.outcome}</p>
            </motion.article>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 sm:mt-24 sm:px-6 lg:px-8">
        <div className="grid gap-6 sm:gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-semibold text-white sm:text-4xl">{t('home.process.title')}</h2>
            <p className="mt-3 max-w-xl text-sm leading-relaxed text-slate-300 sm:mt-4 sm:text-base">{t('home.process.subtitle')}</p>
          </div>

          <div className="grid gap-3 sm:gap-4">
            {steps.map((step, idx) => (
              <div key={step.title} className="rounded-2xl border border-white/10 bg-slate-900/60 p-4 sm:p-5">
                <div className="flex items-start justify-between gap-3 sm:gap-4">
                  ...
                  <span className="rounded-lg border border-emerald-300/35 bg-emerald-500/10 px-2.5 py-1 text-[11px] font-semibold text-emerald-300 sm:px-3 sm:text-xs">
                    {step.metric}
                  </span>
                </div>
              </div>
      ...
      <section className="mx-auto mt-16 max-w-7xl px-4 sm:mt-24 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl border border-violet-300/25 bg-gradient-to-r from-slate-900 via-indigo-950/70 to-slate-900 p-5 sm:p-12">
          ...
          <div className="relative z-10">
            <p className="inline-flex rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-200 sm:px-4 sm:py-1.5">
              {t('home.finalCta.badge')}
            </p>
            <h2 className="mt-4 max-w-3xl text-2xl font-semibold text-white sm:mt-5 sm:text-4xl">{t('home.finalCta.title')}</h2>
            <p className="mt-3 max-w-2xl text-sm leading-relaxed text-slate-200 sm:mt-4 sm:text-base">{t('home.finalCta.subtitle')}</p>
            ...
            <div className="mt-6 flex flex-col gap-2.5 sm:mt-8 sm:gap-3 sm:flex-row">
              ...
              <Link
                ...
                className="inline-flex items-center justify-center rounded-xl border border-white/20 bg-white/5 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/10 sm:px-6 sm:py-3.5"
              >
